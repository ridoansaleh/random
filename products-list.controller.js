...
ProductsListController.$inject = ['$rootScope', '$scope', '$filter', '$log', '$window', '$q', '$stateParams', 'pageType', 'initBrand', 'initTheme', 'ProductService', 'CONFIG', '$state', 'ngDialog', 'localStorageService', 'initCategory', 'PropertiesService', 'sizesData', 'colorsData', 'brandsData', 'MetaService', 'initSize', 'Compare'];
function ProductsListController($rootScope, $scope, $filter, $log, $window, $q, $stateParams, pageType, initBrand, initTheme, ProductService, CONFIG, $state, ngDialog, localStorageService, initCategory, PropertiesService, sizesData, colorsData, brandsData, MetaService, initSize, Compare) {
	
	console.log('===========================================');
	console.log('*. pageType : '+pageType);
	console.log('1. initCategory : '+initCategory);
	// console.log('2. sizesData : ', sizesData);
	// console.log('3. colorsData : ', colorsData);
	// console.log('4. brandsData : ', brandsData);
	console.log('5. initBrand : ', initBrand);
	console.log('6. initSize : '+initSize);
	console.log('7. initTheme : '+initTheme);
	...
	mainList.currentOrder = orderList[mainList.order];
	...
	if (initCategory) {
		...
		mainList.category = initCategory; 
		mainList.activeCategory = initCategory.id; 
		activeCategory = initCategory.id;
		mainList.categoryName = initCategory.name;
		if (mainList.category.banner_url === null) {  
		  mainList.category.banner_url = '/assets/images/placeholder.jpg';  
		}
	...
	mainList.viewPagination = function(pagination) {
		...
		mainList.loading = true;
		return resetList(mainList.category);
	};
	...
	mainList.viewSizeChart = function() {
        return ngDialog.open({
	        template: '/app/product/popup/size_chart.html', 
	...
	getProducts = function(filter, flag) {
		...
		if(pageType===3) {
			var urlParams;
			urlParams = angular.copy(filter);
			urlParams.category = angular.lowercase(initCategory.slug);
	...
	loadProducts = function(filter, flag) {
		...
		// check whether newFilter object has shipping property & if shipping property does exist, 
		// - check whether it's value equals to undefined or blank string
	...
	mainList.getProductsbyfilter = function(data) {
		...
		if(initBrand) {
			console.log("initBrand exist", initBrand.id);
			filter['brands'] = initBrand.id;
		} else {
			console.log("initBrand doesn't exist");
		...
		console.log("getProductsbyfilter : ",filter);
		return loadProducts(filter);
	};
	...
	changeCategory = function(category) {
		...
		mainList.activeCategory = category.id;
	...
	mainList.changeCategory = function(category) { 
	...
	mainList.sortByPopUp = function() {
		return ngDialog.open({
		  template: '/app/product/popup/sort_by.html',
	...
	mainList.categoriesPopUp = function() {
		return ngDialog.open({
		  template: '/app/product/popup/categories.html',
		  className: "ngdialog-theme-oprent",
		  controller: [
		    '$scope', '$window', function($scope, $window) {
	...
	mainList.popUpFilter = function() {
		return ngDialog.open({
		  template: '/app/product/popup/filter.html',
		  className: "ngdialog-theme-oprent",
		  controller: [
		    '$scope', '$window', function($scope, $window) {
		      var firstLoadPopup;
