...
.controller('MainProductCategoryController', MainProductCategoryController);

MainProductCategoryController.$inject = ['$rootScope', '$scope', '$filter', '$log', '$window', '$q', '$stateParams', 'initBrand', 'initTheme', 'ProductService', 'CONFIG', '$state', 'ngDialog', 'localStorageService', 'initCategory', 'PropertiesService', 'sizesData', 'colorsData', 'brandsData', 'MetaService', 'initSize', 'Compare'];
function MainProductCategoryController($rootScope, $scope, $filter, $log, $window, $q, $stateParams, initBrand, initTheme, ProductService, CONFIG, $state, ngDialog, localStorageService, initCategory, PropertiesService, sizesData, colorsData, brandsData, MetaService, initSize, Compare) {
	var activeCategory, 
		changeCategory, 
		childFirstLoad, 
		filter, 
	...
	mainList.viewSizeChart = function() {
        return ngDialog.open({
          template: '/app/product/by-category/html/popup/size_chart.html',
	...
	setColorsFilter = function(colorsList) {
		...
		if (filter.color) {
		  ...
		  $scope.results.colorsParams = popUp.colorsParams;
		} else {
	...
	activeCategory = '';
	if (initCategory) {
		$scope.results.category = initCategory.name;
		...
		return ProductService.getRawList(filter).then(function(data) {
		  ...
		  if (data && data.results.length) {
		    ...
		    if ($stateParams.brands) {
		      $scope.results.brandsParams = popUp.brandsParams;
		      $scope.results.brandsParams.push({
		        id: initBrand.id,
	...
	mainList.categoriesPopUp = function() {
		return ngDialog.open({
		  template: '/app/product/by-category/html/popup/categories.html',
		  className: "ngdialog-theme-oprent",
		  controller: [
		    '$scope', '$window', function($scope, $window) {
	...
	mainList.popUpFilter = function() {
		return ngDialog.open({
		  template: '/app/product/by-category/html/popup/filter.html',
		  className: "ngdialog-theme-oprent",
		  controller: [
		    '$scope', '$window', function($scope, $window) {
		      var firstLoadPopup;
		      ...
		      return $scope.popUpFilter = function(params) {
		        let b, c, j, k, len, len1, ref1, ref2;
		        $scope.products = [];
		        $scope.totalItems = 0;
		        filter['page'] = 1;
		        filter['page_size'] = page_size;
